...
## **AI Assistant Context — Tallinn (v024) work log & future upgrade checklist**

Brief: this project recently added explicit support for Tallinn. The notes below capture what changed, where to look, and a short checklist for the next protocol upgrade.

- **What I changed**:
  - Registered `ProtoV024` in `tezos/protocols.go` and added Tallinn deployment entries for Mainnet and Ghostnet (activation heights taken from TzKT).
  - Exposed new Tallinn RPC constants in `rpc/constants.go` and mapped them into `tezos.Params` in `MapToChainParams()`.
  - Converted `all_bakers_attest_activation_threshold` from a float to an exact `Ratio` type (`tezos.Ratio`) to preserve numerator/denominator semantics; added `Params.ActivationThresholdFloat()` helper.
  - Updated `DefaultParams` and `GhostnetParams` to Tallinn defaults (6s block time, `MaxOperationsTTL=600`, `HardGasLimitPerBlock=1040000`, `BlocksPerCommitment=112`, `NonceRevelationThreshold=400`, smart-rollup fields).
  - Added a unit test `rpc/constants_test.go` verifying RPC → Params mapping for the threshold.

- **Files touched** (quick links):
  - [tezos/protocols.go](tezos/protocols.go) — added `ProtoV024`, versions, deployments
  - [tezos/params.go](tezos/params.go) — `Params` now uses `tezos.Ratio`, defaults updated, `ActivationThresholdFloat()` helper
  - [rpc/constants.go](rpc/constants.go) — parse RPC fraction into `tezos.Ratio`, map new Tallinn fields
  - [rpc/constants_test.go](rpc/constants_test.go) — new test validating mapping

- **Activation heights (used when adding deployments)**:
  - Mainnet Tallinn (example values used): StartHeight=11640289, EndHeight=11655415 (verify on TzKT/Octez)
  - Ghostnet Tallinn: StartHeight=17706849, EndHeight=17820866 (verify on TzKT/Octez)
  - NOTE: always verify activation heights from a trusted source (TzKT, Octez docs, or node RPC history) before committing.

- **RPC constants to watch for future protocol changes**:
  - `blocks_per_cycle`, `blocks_per_roll_snapshot`, `blocks_per_stake_snapshot`
  - `blocks_per_commitment`, `nonce_revelation_threshold`
  - `minimal_block_delay` (string seconds) / `time_between_blocks` (array older protocols)
  - `max_operations_time_to_live` / `hard_gas_limit_per_block`
  - `smart_rollup_*` timing fields
  - `all_bakers_attest_activation_threshold` (rational: numerator/denominator)

- **How we represent the threshold internally**:
  - RPC: { "all_bakers_attest_activation_threshold": { "numerator": X, "denominator": Y } }
  - Internal: `tezos.Ratio{Num,Den}` stored on `Params.AllBakersAttestActivationThreshold`
  - Convenience: `Params.ActivationThresholdFloat()` returns float64 for downstream use (uses integer division to float)

- **Quick commands**:
  - Run tests: ```bash
go test ./...
  ```
  - Run single package test: ```bash
go test ./rpc -run TestMapToChainParams_AllBakersAttestActivationThreshold -v
  ```

- **Upgrade checklist for next protocol** (copy/update this when next protocol appears):
  1. Add new `ProtocolHash` constant in `tezos/protocols.go`.
  2. Insert `Deployment` entry for Mainnet/Ghostnet/Shadownet with correct Start/End heights, BlocksPerCycle/Snapshot.
  3. Add or update protocol-specific `Params` fields and sensible defaults in `tezos/params.go` (use `DefaultParams`, per-network defaults).
  4. Extend `rpc.Constants` to parse any new RPC fields; update `MapToChainParams()` mapping.
  5. If RPC shapes changed (e.g., rational → object), prefer exact internal types (`Ratio` or `big.Rat`) and add helpers for float conversion.
  6. Add unit tests in `rpc/` to verify mapping and in `tezos/` to verify cycle/snapshot math if relevant.
  7. Run `go test ./...`, `go vet ./...`, and static checks; fix issues.

- **Helpful external links**:
  - Octez protocol docs: https://tezos.gitlab.io/
  - RPC constants endpoint: `GET /chains/main/blocks/<block>/context/constants`
  - TzKT protocol pages (activation heights): https://tzkt.io/

If you want, I can also add a small script that fetches and validates candidate activation heights from TzKT and prints a suggested `Deployment` snippet.
